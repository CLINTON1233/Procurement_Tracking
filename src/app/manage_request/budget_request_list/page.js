...
import {
  Plus, Edit, Trash2, Search, RefreshCw, FileSpreadsheet,
  ChevronDown, Grid, CheckCircle, XCircle, Clock, AlertCircle,
  Send, Eye, FileText, Calendar, Package, DollarSign, Wallet,
  ArrowUp, ArrowDown, List as ListIcon, RotateCcw, Server, Layers,
  TrendingUp,
} from "lucide-react";
import {
...
export default function BudgetRequestListPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      Swal.fire({ title: "Error!", text: "Failed to fetch request data", icon: "error", confirmButtonColor: "#1e40af" });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleRevision = (request) => router.push(`/manage_revision/revision/${request.id}`);

  const handleDeleteRequest = async (request) => {
    const result = await Swal.fire({
      title: "Delete Request?", text: "This action cannot be undone!", icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({ title: "Deleted!", icon: "success", timer: 1500, confirmButtonColor: "#1e40af" });
        fetchData();
      } catch (error) {
  ...
  const toggleSelectMode = () => { setSelectMode(!selectMode); setSelectedRequests([]); setSelectAll(false); };
  const handleSelectRequest = (id) => setSelectedRequests(p => p.includes(id) ? p.filter(x => x !== id) : [...p, id]);
  const handleSelectAll = () => { if (selectAll) setSelectedRequests([]); else setSelectedRequests(filteredRequests.map(r => r.id)); setSelectAll(!selectAll); };

  const handleBulkDelete = async () => {
    if (!selectedRequests.length) return Swal.fire({ title: "No Selection", text: "Please select at least one request", icon: "warning", confirmButtonColor: "#1e40af" });
    const result = await Swal.fire({
      title: `Delete ${selectedRequests.length} Requests?`, icon: "warning", showCancelButton: true,
      confirmButtonText: "Yes, Delete All!", buttonsStyling: false,
    ...
    if (result.isConfirmed) {
      Swal.fire({ title: "Deleting...", allowOutsideClick: false, didOpen: () => Swal.showLoading() });
      let success = 0;
      for (const id of selectedRequests) { try { await budgetService.deleteRequest(id); success++; } catch {} }
      Swal.fire({ title: "Deleted!", text: `${success} Requests deleted`, icon: "success", confirmButtonColor: "#1e40af" });
      fetchData(); setSelectMode(false); setSelectedRequests([]); setSelectAll(false);
    }
  };
  ...
  const getStatusConfig = (status) => {
    const map = {
      DRAFT: { bg: "bg-gray-100", text: "text-gray-600", dot: "#9ca3af", label: "Draft", icon: FileText },
      SUBMITTED: { bg: "bg-blue-100", text: "text-blue-700", dot: "#2563eb", label: "Submitted", icon: Send },
      BUDGET_APPROVED: { bg: "bg-emerald-100", text: "text-emerald-700", dot: "#10b981", label: "Approved", icon: CheckCircle },
      BUDGET_REJECTED: { bg: "bg-red-100", text: "text-red-700", dot: "#ef4444", label: "Rejected", icon: XCircle },
      WAITING_SR_MR: { bg: "bg-purple-100", text: "text-purple-700", dot: "#8b5cf6", label: "Waiting SR/MR", icon: Clock },
    };
    return map[status] || { bg: "bg-gray-100", text: "text-gray-600", dot: "#9ca3af", label: status, icon: AlertCircle };
  };
  ...
  const filteredRequests = useMemo(() => {
    ...
    if (sorting.length > 0) {
      ...
      filtered.sort((a, b) => {
        let av = a[id], bv = b[id];
        if (id === "estimated_total") { av = Number(av || 0); bv = Number(bv || 0); }
            ...
            <div className="space-y-3">
              <div>
                ...
                <StackedBar segments={[
                  ...
                  { pct: Math.max(Math.round(approvalRate), 1), color: "#10b981" },
            ...
            <div className="flex flex-col gap-4">
                  ...
                  {selectMode && (
                    <>
                      ...
                      <button onClick={handleBulkDelete}
                        ...
                        <Trash2 className="w-3.5 h-3.5" />Delete ({selectedRequests.length})
                      </button>
                    </>
                  )}
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="relative flex-1 min-w-[200px]">
                  ...
                  <input type="text" placeholder="Search request no, requester, item, department..."
                  ...
                  {[
                    ...
                    <button key={s.val} onClick={() => setStatusFilter(s.val)}
                      className={`px-3 py-2 text-xs font-medium rounded-lg transition-colors ${statusFilter === s.val
                        ? s.val === "BUDGET_APPROVED" ? "bg-emerald-600 text-white"
                        : s.val === "BUDGET_REJECTED" ? "bg-red-600 text-white"
                        : s.val === "SUBMITTED" ? "bg-blue-600 text-white"
                        : s.val === "WAITING_SR_MR" ? "bg-purple-600 text-white"
                        : s.val === "DRAFT" ? "bg-gray-500 text-white"
                  ...
                  {["all", "ITEM", "SERVICE"].map(type => (
                    <button key={type} onClick={() => setTypeFilter(type)}
                      className={`px-3 py-2 text-xs font-medium rounded-lg transition-colors ${typeFilter === type
                        ? type === "ITEM" ? "bg-[#1e3a5f] text-white" : type === "SERVICE" ? "bg-blue-600 text-white" : "bg-gray-800 text-white"
                        : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
